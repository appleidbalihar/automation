...
**First installation on `rapidrag.ai` production server.**
...
2. [Step 0 — Scan Existing Server (Shared Host)](#step-0--scan-existing-server-shared-host)
3. [Step 1 — Configure Docker Data Root](#step-1--configure-docker-data-root)
4. [Step 2 — Clone the Repository](#step-2--clone-the-repository)
5. [Step 3 — Create Production `.env.production`](#step-3--create-production-envproduction)
6. [Step 4 — Bootstrap Vault and Seed All Secrets](#step-4--bootstrap-vault-and-seed-all-secrets)
7. [Step 5 — Build Images](#step-5--build-images)
8. [Step 6 — Start the Full Stack](#step-6--start-the-full-stack)
9. [Step 7 — Post-Start: Keycloak Admin User](#step-7--post-start-keycloak-admin-user)
10. [Step 8 — Post-Start: n8n Owner Setup](#step-8--post-start-n8n-owner-setup)
11. [Step 9 — Post-Start: Dify Admin Setup](#step-9--post-start-dify-admin-setup)
12. [Step 10 — Seed Global App Credentials (Dify + LLM)](#step-10--seed-global-app-credentials-dify--llm)
13. [Step 11 — Configure OAuth Callback URLs](#step-11--configure-oauth-callback-urls)
14. [Step 12 — Install SSL Certificate](#step-12--install-ssl-certificate)
15. [Step 13 — Configure Outer Nginx](#step-13--configure-outer-nginx)
16. [Step 14 — Smoke Tests](#step-14--smoke-tests)
17. [Step 15 — Enable SSL Auto-Renewal](#step-15--enable-ssl-auto-renewal)
18. [Step 16 — Install Systemd Service](#step-16--install-systemd-service)
19. [Step 17 — Generate Service Credentials Document](#step-17--generate-service-credentials-document)
20. [Service Access Reference](#service-access-reference)
21. [How to View / Rotate Secrets](#how-to-view--rotate-secrets)
...
- [ ] `rapidrag.ai` DNS A record points to server's public IP
- [ ] Ports 80 and 443 are open in the firewall; ports 3443 and 8443 are NOT public (Keycloak is proxied through nginx at /auth/)
...
## Step 0 — Scan Existing Server (Shared Host)

> **This server already runs other applications.** Before assigning any host ports, scan what is already in use and plan around conflicts. Run this entire section first and record the results.
...
sudo ss -tlnup
# or
sudo netstat -tlnup 2>/dev/null || sudo ss -tlnup

# Ports bound by Docker containers specifically
docker ps --format "table {{.Names}}\t{{.Ports}}" 2>/dev/null || true

# Nginx virtual hosts already configured
ls -1 /etc/nginx/sites-enabled/ 2>/dev/null || ls -1 /etc/nginx/conf.d/ 2>/dev/null
```

### 0.2 — Key ports used by RapidRAG (all internal except 80/443)

| Port | Bound to | Service | Notes |
|------|----------|---------|-------|
| 80 | all interfaces | Host nginx | Shared with other vhosts — add `server_name rapidrag.ai` block only |
| 443 | all interfaces | Host nginx | Same — add a new `server {}` block, do NOT replace existing configs |
| 8443 | **127.0.0.1 only** | Keycloak | Not public — proxied by nginx at `/auth/` |
| 3443 | **127.0.0.1 only** | Inner Docker nginx (web) | Not public |
| 5679 | **127.0.0.1 only** | n8n | Proxied at `/n8n/` |
| 8200 | Docker network only | Vault | SSH tunnel for admin access |
| 5432 | Docker network only | PostgreSQL | SSH tunnel only |
| 4000 | Docker network only | API Gateway | Proxied at `/rapidrag/connect/` |

### 0.3 — Nginx co-existence rule

The nginx config at `infra/nginx/rapidrag-production.conf` uses `server_name rapidrag.ai` and does **not** touch any other `server_name`. Place it as a new file in `sites-available/` and symlink — **never edit** the default or existing application configs.

```bash
# Check what server_names are already configured — ensure no clash
grep -r "server_name" /etc/nginx/sites-enabled/ /etc/nginx/conf.d/ 2>/dev/null
```

### 0.4 — Systemd services inventory

```bash
# List running services to avoid name clashes with the rapidrag service
systemctl list-units --type=service --state=running | grep -v "@"

# Confirm rapidrag service is not already installed
systemctl status rapidrag 2>&1 | head -5
```
...
```bash
df -h /home /opt /
# RapidRAG needs ≥ 100 GB free on /home (Docker data root + volumes)
# Docker images alone are ~3-4 GB; Dify + OpenSearch volumes can reach 20+ GB
```

> **If any port conflict is found:** adjust the Docker host port mapping in `docker-compose.prod.yml` for the conflicting service, and update the `proxy_pass` in `rapidrag-production.conf` to match. Do NOT change internal Docker network ports.
...
docker exec -e VAULT_TOKEN="${VAULT_ROOT_TOKEN}" \
  $(docker ps -qf name=rapidrag-vault-1) \
...
> **Alternative:** Open `http://localhost:3002` (or `https://rapidrag.ai/dify/` once nginx is configured) and complete the browser wizard instead.
...
```bash
sudo certbot certonly --standalone -d rapidrag.ai
# Certs will be at:
# /etc/letsencrypt/live/rapidrag.ai/fullchain.pem
# /etc/letsencrypt/live/rapidrag.ai/privkey.pem
```
...
sudo ln -s /etc/nginx/sites-available/rapidrag-production \
           /etc/nginx/sites-enabled/rapidrag-production
...
sudo systemctl enable rapidrag

# Start immediately (stack must already be up from Step 6, or start fresh here)
sudo systemctl start rapidrag
...
## Step 17 — Generate Service Credentials Document
...
```
Internet (Cloudflare → rapidrag.ai)
    │
    ▼
